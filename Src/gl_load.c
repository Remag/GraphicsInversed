...
static void ParseVersionFromString(int *pOutMajor, int *pOutMinor, const char *strVersion)
{
	...
	*pOutMajor = 0;
	...
	strncpy_s(strWorkBuff, 10, strVersion, iLength);
	...
	*pOutMajor = atoi(strWorkBuff);
	...
	if(!strDotPos)
	{
		...
		strcpy_s(strWorkBuff, 10, strVersion + iLength + 1);
	}
	else
	{
		...
		strncpy_s(strWorkBuff, 10, strVersion + iLength + 1, iLengthMinor);
...
static void ProcExtsFromExtString(const char *strExtList)
{
	...
	while(*strCurrPos)
	{
		...
		strncpy_s(strWorkBuff, 256, strCurrPos, iStrLen);
