...
{
	doWrite( file.Ptr(), PNG_FORMAT_RGBA, 0, imageSize );
}

void CPngFile::Write( CArrayView<BYTE> file, TTexelFormat format, int rowStride, CVector2<int> imageSize )
{
	const auto libFormat = findLibPngFormat( format );
	doWrite( file.Ptr(), libFormat, rowStride, imageSize );
}

unsigned CPngFile::findLibPngFormat( TTexelFormat format ) const
{
	switch( format ) {
		case TF_BGR:
